...
export default function Home() {
  ...
  return <main>
    ...
    <section id="inicio" className="hero hero-simple"><div className="hero-media" aria-hidden="true">{heroVideoUrl ? <video src={heroVideoUrl} poster="/hero-solar-v1.png" autoPlay muted loop playsInline preload="auto" onError={() => setHeroVideoUrl('')} /> : <img src="/hero-solar-v1.png" alt="" />}</div><div className="hero-shade" aria-hidden="true"/><div className="hero-copy"><span className="eyebrow"><MapPin /> {texts.heroEyebrow}</span>{projectName && <span className="project-badge">{projectName}</span>}<h1>{texts.heroTitle}</h1><p>{texts.heroSubtitle}</p><div className="hero-offers"><span><b>{texts.offerOne}</b></span><span><b>{texts.offerTwo}</b></span></div><div className="hero-actions"><a className="btn primary" href="/agendar-visita">AGENDAR VISITA <CalendarDays /></a></div><div className="trust"><span><ShieldCheck /> Proceso transparente</span><span><Trees /> Excelente ubicación</span><span><CalendarDays /> Visitas personalizadas</span></div></div></section>
