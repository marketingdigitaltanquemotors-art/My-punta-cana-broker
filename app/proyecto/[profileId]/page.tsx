...
export default function ProjectProfilePage({ params }: { params: { profileId: string } }) {
  ...
  return <main>
    ...
    <section id="inicio" className="hero hero-simple"><div className="hero-media" aria-hidden="true">{heroVideoUrl ? <video src={heroVideoUrl} poster="/hero-solar-v1.png" autoPlay muted loop playsInline onError={() => setHeroVideoUrl('')} /> : <img src="/hero-solar-v1.png" alt="" />}</div><div className="hero-shade" aria-hidden="true"/><div className="hero-copy"><span className="eyebrow"><MapPin /> PUNTA CANA · BÁVARO</span>{projectName && <span className="project-badge">{projectName}</span>}<h1>Encuentra tu solar en el centro de <em>Punta Cana-Bávaro, cerca de todo.</em></h1><p>Agenda una visita personalizada en Punta Cana-Bávaro de manera rápida y sencilla.</p><div className="hero-offers"><span><b>Desde un 15%</b></span><span><b>Desde 150$</b> por mentro</span></div><div className="hero-actions"><a className="btn primary" href={scheduleHref}>AGENDAR VISITA <CalendarDays /></a><a className="btn outline" href="/">VER WEB PRINCIPAL <ArrowLeft /></a></div><div className="trust"><span><ShieldCheck /> Proceso transparente</span><span><Trees /> Ubicaciones con potencial</span><span><CalendarDays /> Visitas personalizadas</span></div></div></section>
    <section className="built-homes"><div><span className="kicker"><HomeIcon /> CASAS CONSTRUIDAS</span><h2>Imagina tu casa hecha realidad en Punta Cana-Bávaro.</h2><p>Estos solares son una oportunidad para construir cerca de todo, con una visión clara de comunidad, acceso y futuro crecimiento.</p><a className="btn primary" href={scheduleHref}>AGENDAR VISITA <ArrowRight /></a></div><img src="/casas-construidas-v1.png" alt="Casas modernas construidas en un residencial tropical" /></section>
    <section className="content-section" id="solares"><div className="content-heading"><span className="kicker"><ImageIcon /> SOLARES</span><h2>{projectName ? `Fotos de ${projectName}` : 'Fotos de solares disponibles'}</h2></div>{solares.length ? <div className="media-grid">{solares.map(item => <article className="media-card" key={item.id}><img src={item.image_url} alt={item.title} /><div><h3>{item.title}</h3>{item.description && <p>{item.description}</p>}</div></article>)}</div> : <div className="empty-content">Muy pronto verás nuevas fotos de solares.</div>}</section>
    {testimonialsEnabled && <section className="content-section testimonials" id="testimonios"><div className="content-heading"><span className="kicker"><Star /> TESTIMONIOS</span><h2>Testimonio de clientes en sus solares</h2></div>{activeTestimonial ? <div className="testimonial-carousel" onTouchStart={event => setTouchStart(event.touches[0].clientX)} onTouchEnd={event => finishSwipe(event.changedTouches[0].clientX)}><button className="carousel-arrow" type="button" onClick={previousTestimonial} aria-label="Ver testimonio anterior"><ArrowLeft /></button><article className="testimonial-feature" key={activeTestimonial.id}><img src={activeTestimonial.image_url} alt={activeTestimonial.title} /><div><span>Cliente {testimonialIndex + 1} de {testimonios.length}</span><h3>{activeTestimonial.title}</h3>{activeTestimonial.description && <p>{activeTestimonial.description}</p>}</div></article><button className="carousel-arrow" type="button" onClick={nextTestimonial} aria-label="Ver siguiente testimonio"><ArrowRight /></button><div className="carousel-dots">{testimonios.map((item, index) => <button key={item.id} type="button" className={index === testimonialIndex ? 'active' : ''} onClick={() => setTestimonialIndex(index)} aria-label={`Ver testimonio de ${item.title}`} />)}</div></div> : <div className="empty-content">Muy pronto compartiremos testimonios de nuestros clientes.</div>}</section>}
    <section className="visit-banner"><div><span className="kicker">CONOCE EL LUGAR</span><h2>Tu próximo proyecto comienza con una visita.</h2></div><a className="btn primary" href={scheduleHref}>VER FECHAS DISPONIBLES <ArrowRight /></a></section>
    <footer id="contacto"><div className="footer-grid"><div><Brand /><p>Solares y terrenos en Punta Cana-Bávaro</p></div><div><b>CONTÁCTANOS</b><a href="https://wa.me/18097479704"><MessageCircle/> WhatsApp 809-747-9704</a><a href="tel:+18097479704"><Phone/> Teléfono 809-747-9704</a><a href="mailto:info@mypuntacanabroker.com"><Mail/> Correo electrónico</a></div><div><b>SÍGUENOS</b><a href="https://www.instagram.com/mypuntacanabroker/" target="_blank" rel="noopener noreferrer">◎&nbsp;&nbsp; Instagram</a><a href="https://www.facebook.com/mypuntacanabroker/" target="_blank" rel="noopener noreferrer">f&nbsp;&nbsp; Facebook</a><a href="#"><MapPin/> Ubicación</a></div></div><div className="copyright"><span>© 2026 My Punta Cana Broker</span><span>Invierte en tierra. Construye tu futuro.</span></div></footer>
  </main>;
}
