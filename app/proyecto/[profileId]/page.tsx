...
export default function ProjectProfilePage() {
  ...
  const scheduleHref = profileId ? `/agendar-visita?profileId=${encodeURIComponent(profileId)}` : '/agendar-visita';
  ...
  return <main>
    ...
    <section id="inicio" className="hero hero-simple"><div className="hero-media" aria-hidden="true">{heroVideoUrl ? <ViewportVideo src={heroVideoUrl} poster="/hero-solar-v1.png" onError={() => setHeroVideoUrl('')} /> : <img src="/hero-solar-v1.png" alt="" />}</div><div className="hero-shade" aria-hidden="true"/><div className="hero-copy"><span className="eyebrow"><MapPin /> {texts.heroEyebrow}</span>{projectName && <span className="project-badge">{projectName}</span>}<h1>{texts.heroTitle}</h1><p>{texts.heroSubtitle}</p><div className="hero-offers"><span><b>{texts.offerOne}</b></span><span><b>{texts.offerTwo}</b></span></div><div className="hero-actions"><a className="btn primary" href={scheduleHref}>AGENDAR VISITA <CalendarDays /></a><a className="btn outline" href="/">VER WEB PRINCIPAL <ArrowLeft /></a></div><div className="trust"><span><ShieldCheck /> Proceso transparente</span><span><Trees /> Excelente ubicación</span><span><CalendarDays /> Visitas personalizadas</span></div></div></section>
    <section className="built-homes"><div><span className="kicker"><HomeIcon /> {texts.builtKicker}</span><h2>{texts.builtTitle}</h2><p>{texts.builtText}</p><a className="btn primary" href={scheduleHref}>AGENDAR VISITA <ArrowRight /></a></div><BuiltHomesCarousel images={builtImageUrls} /></section>
