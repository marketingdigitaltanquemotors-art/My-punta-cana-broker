...
import { ArrowLeft, CalendarDays, Check, Clock3, LoaderCircle, MapPin, MessageCircle, ShieldCheck } from 'lucide-react';
import { Calendar } from '@/components/ui/calendar';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';

const lots = ['A-12', 'B-07', 'C-21'];
...
type Confirmation = { lotId: string; date: string; time: string; name: string; code: string };

export default function ScheduleVisit() {
  const today = useMemo(() => { const value = new Date(); value.setHours(0,0,0,0); return value; }, []);
  const [date, setDate] = useState<Date>(today);
  const [lotId, setLotId] = useState('A-12');
  ...
  useEffect(() => { const requested = new URLSearchParams(window.location.search).get('solar'); if (requested && lots.includes(requested)) setLotId(requested); }, []);
  useEffect(() => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/appointments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lotId, date: dateKey(date), time, ...details }) });
  ...
  const whatsapp = confirmation ? encodeURIComponent(`Hola, My Punta Cana Broker. Acabo de agendar una visita para conocer el Solar #${confirmation.lotId} el día ${confirmation.date} a las ${showTime(confirmation.time)}. Mi código de reserva es ${confirmation.code}.`) : '';

  return <main className="schedule-page">
    <header className="schedule-header"><a href="/" className="brand"><span className="logo-mark">M</span><span><b>MY PUNTA CANA</b><small>BROKER</small></span></a><a href="/" className="back-link"><ArrowLeft/> VOLVER AL INICIO</a></header>
    ...
    <section className="schedule-content">
      {confirmation ? <div className="schedule-confirmation"><span className="check"><Check/></span><span className="kicker">RESERVA CONFIRMADA</span><h2>¡Tu visita con My Punta Cana Broker ha sido agendada!</h2><p>Te esperamos para conocer el Solar #{confirmation.lotId}.</p><div className="confirmation-details"><span><MapPin/><small>Solar</small><b>#{confirmation.lotId}</b></span><span><CalendarDays/><small>Fecha</small><b>{format(new Date(`${confirmation.date}T12:00:00`), "d 'de' MMMM", { locale: es })}</b></span><span><Clock3/><small>Hora</small><b>{showTime(confirmation.time)}</b></span></div><p className="reservation-code">Código de reserva <b>{confirmation.code}</b></p><a className="btn whatsapp" target="_blank" rel="noreferrer" href={`https://wa.me/?text=${whatsapp}`}><MessageCircle/> ENVIAR POR WHATSAPP</a><button className="new-booking" onClick={() => { setConfirmation(null); setDetails({name:'',phone:'',email:''}); setTime(''); }}>AGENDAR OTRA VISITA</button></div> : <form className="scheduler" onSubmit={submit}>
        ...
        <div className="schedule-step details-step"><span className="step-number">03</span><div><span className="kicker">DATOS DE CONTACTO</span><h2>Completa tu reserva</h2></div><div className="schedule-fields"><label>Solar<select value={lotId} onChange={e => setLotId(e.target.value)}>{lots.map(lot => <option value={lot} key={lot}>Solar #{lot}</option>)}</select></label><label>Nombre completo<Input required value={details.name} onChange={e => setDetails({...details,name:e.target.value})} placeholder="Tu nombre" /></label><label>WhatsApp<Input required type="tel" value={details.phone} onChange={e => setDetails({...details,phone:e.target.value})} placeholder="(809) 000-0000" /></label><label>Correo electrónico <small>(opcional)</small><Input type="email" value={details.email} onChange={e => setDetails({...details,email:e.target.value})} placeholder="tu@correo.com" /></label></div>{error && <p className="schedule-error" role="alert">{error}</p>}<Button type="submit" disabled={submitting || !time} className="confirm-booking">{submitting ? <><LoaderCircle/> CONFIRMANDO…</> : <>CONFIRMAR VISITA <Check/></>}</Button><p className="privacy"><ShieldCheck/> Al confirmar, este horario quedará bloqueado para otros clientes.</p></div>
      </form>}
    </section>
  </main>;
}
